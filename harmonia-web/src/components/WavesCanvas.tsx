...
/**
 * Animated waveform + particle background used on every page.
 * Extracted from the original landing so the look stays consistent
 * across the site (landing, project page, …).
 */
export default function WavesCanvas() {
    ...
    const draw = () => {
      ...
      lines.forEach((ln, idx) => {
        ...
        for (let x = 0; x <= width; x += 6) {
          const y =
            yBase +
            Math.sin(x * ln.freq + t * ln.speed * 8 + ln.phase) * ln.amp +
            Math.sin(x * ln.freq * 0.4 + t * ln.speed * 3) * ln.amp * 0.35
        ...
        ctx.shadowColor = isAccent ? 'rgba(78,161,255,0.55)' : 'transparent'
  ...
  return (
    <canvas ref={canvasRef} className="harmonia-canvas" aria-hidden="true" />
  )
}
